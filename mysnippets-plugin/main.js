...
function snippetsMenu(app, plugin, settings) {
    ...
    const existing = document.querySelector(".menu.MySnippets-statusbar-menu");
    if (existing) {
        existing.remove();
        return;
    }
    ...
    // Build a fully custom DOM menu so we control all click behaviour
    const menuDom = document.createElement("div");
    menuDom.addClass("menu");
    ...
    // Position it above the status bar
    menuDom.style.position = "fixed";
    menuDom.style.zIndex = "9999";
    const windowX = window.innerWidth;
    const windowY = window.innerHeight;
    menuDom.style.right = "15px";
    menuDom.style.bottom = "37px";
    menuDom.style.left = "auto";
    menuDom.style.top = "auto";
    ...
    currentSnippets.forEach((snippet) => {
        ...
        titleEl.addClass("menu-item-title");
        titleEl.setText(snippet);
        ...
        const buttonComponent = new obsidian.ButtonComponent(btnWrapper);
        buttonComponent
            .setIcon("ms-snippet")
    ...
    menuDom.appendChild(sep);

    // Actions row
    const actionsItem = document.createElement("div");
    actionsItem.addClass("menu-item");
    actionsItem.addClass("buttonitem");

    const actionsTitle = document.createElement("span");
    actionsTitle.addClass("menu-item-title");
    actionsTitle.setText("Actions");
    actionsTitle.setAttribute("style", "font-weight: 700");
    actionsItem.appendChild(actionsTitle);

    const reloadWrapper = document.createElement("div");
    const reloadButton = new obsidian.ButtonComponent(reloadWrapper);
    reloadButton.setIcon("ms-reload").setClass("MySnippetsButton").setClass("MS-Reload").setTooltip("Reload snippets");
    reloadWrapper.setAttribute("style", "margin-right: 3px");
    reloadWrapper.addEventListener("mousedown", (e) => {
        e.stopPropagation();
        customCss.requestLoadSnippets();
        new obsidian.Notice("Snippets reloaded");
    });
    actionsItem.appendChild(reloadWrapper);

    const folderWrapper = document.createElement("div");
    const folderButton = new obsidian.ButtonComponent(folderWrapper);
    folderButton.setIcon("ms-folder").setClass("MySnippetsButton").setClass("MS-Folder").setTooltip("Open snippets folder");
    folderWrapper.addEventListener("mousedown", (e) => {
        e.stopPropagation();
        app.openWithDefaultApp(snippetsFolder);
    });
    actionsItem.appendChild(folderWrapper);

    const addWrapper = document.createElement("div");
    const addButton = new obsidian.ButtonComponent(addWrapper);
    addButton.setIcon("ms-add").setClass("MySnippetsButton").setClass("MS-Folder").setTooltip("Create new snippet");
    addWrapper.setAttribute("style", "margin-left: 3px");
    addWrapper.addEventListener("mousedown", (e) => {
        e.stopPropagation();
        menuDom.remove();
        document.removeEventListener("mousedown", closeMenu, true);
        new CreateSnippetModal(app, plugin).open();
    });
    actionsItem.appendChild(addWrapper);

    menuDom.appendChild(actionsItem);

    document.body.appendChild(menuDom);
    // Register close-on-outside-click after a tick so the current click doesn't immediately close it
    setTimeout(() => {
